...
class KS:
    ...
    def setup_etdrk4(self):
        ...
        self.E2 = np.exp(self.dt * self.l / 2.0)
        self.M = 16  # no. of points for complex means
        self.r = np.exp(
            1j * pi * (np.r_[1 : self.M + 1] - 0.5) / self.M
        )  # roots of unity
        self.LR = self.dt * np.repeat(
            self.l[:, np.newaxis], self.M, axis=1
        ) + np.repeat(self.r[np.newaxis, :], self.N, axis=0)
        self.Q = self.dt * np.real(np.mean((np.exp(self.LR / 2.0) - 1.0) / self.LR, 1))
        self.f1 = self.dt * np.real(
            np.mean(
                (
                    -4.0
                    - self.LR
                    + np.exp(self.LR) * (4.0 - 3.0 * self.LR + self.LR ** 2)
                )
                / (self.LR ** 3),
                1,
            )
        ...
        self.f2 = self.dt * np.real(
            np.mean(
                (2.0 + self.LR + np.exp(self.LR) * (-2.0 + self.LR)) / (self.LR ** 3), 1
            )
        )
        ...
        if v0 is None:
            ...
            if u0 is None:
                ...
                if testing:
                    ...
                    u0 = np.cos(self.x / self.L) * (1.0 + np.sin(self.x / self.L))
                else:
                    # random noise
